...
function SplashCursor({
  ...
  BACK_COLOR = { r: 0.04, g: 0.01, b: 0.2 },
  TRANSPARENT = true,
  RAINBOW_MODE = false,
  COLOR = '#A855F7',
}: {
  ...
  useEffect(() => {
    ...
    function getUniforms(program: WebGLProgram) {
      ...
      for (let i = 0; i < uniformCount; i++) {
        ...
        let uniformName = activeInfo.name;
    ...
    const advectionShader = compileShader(
      ...
      `,
      (ext.supportLinearFiltering ? undefined : ['MANUAL_FILTERING']) as string[] | undefined
    );
    ...
    function generateColor() {
      if (!config.RAINBOW_MODE) {
        const index = Math.floor(colorUpdateTimer * palette.length) % palette.length;
